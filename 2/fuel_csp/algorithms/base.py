"""Shared types for the solver implementations."""
...
@dataclass
class SolverStats:
    """Metrics gathered during one solver run.

    These are the same fields the assignment requires us to compare across
    algorithms (backtracks, nodes expanded, time, ...).
    """
...
@dataclass
class SolverResult:
    """Final output bundle. assignment can be partial (COP)."""
    ...
    def pretty(self) -> str:
        s = self.stats
        return (
            f"{s.algorithm:<22s} | N={s.n:>3d} "
            f"| J(S)={s.objective:>9.2f} "
            f"| assigned={s.num_assigned:>3d}/{s.n:<3d} "
            f"| backtracks={s.backtracks:>6d} "
            f"| nodes={s.nodes_expanded:>6d} "
            f"| t={s.runtime_seconds*1000:>7.2f}ms"
        )


class Timer:
    """Context manager that writes elapsed seconds into SolverStats."""
...
class Solver:
    """Abstract base. Subclasses set `name` and implement `solve`."""
    ...
    def solve(self, problem: Problem) -> SolverResult:  # pragma: no cover - abstract
